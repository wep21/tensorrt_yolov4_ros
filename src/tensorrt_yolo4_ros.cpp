...
#include "tensorrt_yolo4_ros.h"

TensorrtYoloROS::TensorrtYoloROS(/* args */) : pnh_("~")
{
  std::string package_path = ros::package::getPath("tensorrt_yolo4");
  std::string engine_path = package_path + "/data/yolov4_608.engine";
...
void TensorrtYoloROS::createROSPubSub()
{
  ...
  pub_image_ = nh_.advertise<sensor_msgs::Image>("/perception/tensorrt_yolo4/classified_image", 1);
}
...
void TensorrtYoloROS::imageCallback(const sensor_msgs::Image::ConstPtr & in_image_msg)
{
  ...
  std::cout << "exec time: " << total << " ms." << std::endl;
